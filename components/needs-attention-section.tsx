        ...
        <Card className="border-border shadow-sm overflow-hidden h-full mt-8">
            <CardHeader className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border/60 bg-muted/10 py-5">
                ...
                {atRiskContributors.length > 0 && (
                    <Badge variant="outline" className="bg-rose-50 dark:bg-rose-500/10 text-rose-700 dark:text-rose-400 border-rose-200 dark:border-rose-500/20 px-3 py-1 text-sm shadow-sm">
            ...
            <CardContent className="p-0 flex flex-col min-h-[300px] bg-background">
                {atRiskContributors.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-16 gap-4 bg-muted/5">
                        ...
                        <div className="relative">
                            <div className="p-6 flex flex-col gap-4 bg-muted/5">
                                {visible.map((person: ContributorSum, i: number) => {
                                    ...
                                    const cardBg = isCritical ? 'bg-rose-50 dark:bg-rose-500/5 border-rose-200 dark:border-rose-500/20' : 'bg-amber-50 dark:bg-amber-500/5 border-amber-200 dark:border-amber-500/20';
                                        ...
                                        >
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                {person.objectives && person.objectives.length > 0 && (
                                                    <div className="flex flex-col gap-2.5 mt-2 bg-background/50 p-3 rounded-xl border border-border/50">
                                                        {person.objectives.slice(0, 2).map((okr: ContributorSumObj, oi: number) => (
                                                            <div key={oi} className="flex items-center gap-3">
                                                                ...
                                                                <span className="text-sm text-muted-foreground shrink-0 tabular-nums font-bold bg-background px-1.5 py-0.5 rounded-md border border-border">{okr.progress?.toFixed(0)}%</span>
                                                            </div>
                                                        ))}
                            ...
                            {!showAll && hiddenCount > 0 && (
                                <div className="relative mt-[-80px] px-6 pb-2">
                                    ...
                                    <div className="bg-background pt-2 pb-6 flex justify-center">
                                        <Button
                                            variant="outline"
                            ...
                            {showAll && atRiskContributors.length > INITIAL_SHOW && (
                                <div className="px-6 pb-8 bg-muted/5 flex justify-center">
                                    <Button
                                        variant="outline"
