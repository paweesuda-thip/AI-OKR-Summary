...
export const EvervaultCard = ({
  ...
  // Memoize random string generation to avoid cascading updates in effect
  const initialRandomString = useMemo(() => generateRandomString(1500), []);
  
  useEffect(() => {
    setRandomString(initialRandomString);
  }, [initialRandomString]);
  ...
  return (
    ...
    >
      <div
        ...
        className="group/card rounded-3xl w-full relative overflow-visible bg-transparent flex items-center justify-center h-full"
      >
        <CardPattern
          mouseX={mouseX}
          mouseY={mouseY}
        ...
        <div className="relative z-10 flex items-center justify-center overflow-visible">
          <div className="relative h-44 w-44 rounded-full flex items-center justify-center text-white font-bold text-4xl overflow-visible">
            ...
            {children && <div className="z-20 absolute inset-0 flex items-center justify-center">{children}</div>}
          </div>
        </div>
      </div>
    </div>
