...
import type { ILlmProvider, LlmTextRequest } from "@/src/Domain/Interfaces/ILlmProvider";
import { LlmProviderError, MissingConfigurationError } from "@/src/Domain/Exceptions";
...
/**
 * Thin adapter that exposes the legacy `geminiService` object through the
 * domain-level `ILlmProvider` contract.
 *
 * The rich OKR-specific prompt helpers (`generateSummary`, `askQuestion`, ...)
 * remain on `geminiService` for backward compatibility with the UI code that
 * calls them directly today. New code should depend on `ILlmProvider` instead.
 */
export class GeminiLlmProvider implements ILlmProvider {
  ...
  async generateText(req: LlmTextRequest): Promise<string> {
    if (!this.isConfigured()) {
      throw new MissingConfigurationError('NEXT_PUBLIC_GEMINI_API_KEY');
    }
    try {
      const response = await ai.models.generateContent({
        model: CURRENT_MODEL,
        contents: [
          ...(req.system ? [{ role: 'user', parts: [{ text: req.system }] }] : []),
      ...
      throw new LlmProviderError(
        err instanceof Error ? err.message : 'Gemini generateText failed',
        'gemini',
        err,
      );
    }
  ...
  async generateJson<T = unknown>(
    req: LlmTextRequest & { maxAttempts?: number; label?: string },
  ): Promise<T | null> {
