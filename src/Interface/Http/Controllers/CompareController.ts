/* eslint-disable @typescript-eslint/no-explicit-any -- verbatim move from the former compare/route.ts; any-usage is pre-existing and behaviour is preserved as-is. */
import 'server-only';
 ...
 *
 * Moved verbatim from the former `app/api/compare/route.ts`. Orchestrates
 * two parallel LLM calls (rounds + summary) with custom retry + JSON-repair
 * logic. Future work: extract the JSON-repair helpers into an
 * `AnthropicLlmProvider` and the prompt-builder into a
 * `CompareOkrPlayersUseCase`.
 */
