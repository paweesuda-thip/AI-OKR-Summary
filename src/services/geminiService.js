...
`;
        contributors.slice(0, 5).forEach((person) => {
...
`;
        objectives.slice(0, 8).forEach((obj) => {
...
const geminiService = {
    ...
    async generateTopPerformersSummary(top3) {
        if (!this.isConfigured()) {
            throw new Error('กรุณาตั้งค่า VITE_GEMINI_API_KEY ในไฟล์ .env');
        }

        const personData = top3.map((person, idx) => ({
            rank: idx + 1,
            name: person.fullName,
            score: person.totalPointCurrent,
            krCount: person.krCount,
            objectives: (person.objectives || []).map(o => ({
